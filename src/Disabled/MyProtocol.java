...
<<<<<<< Updated upstream:src/MyProtocol.java
		...
		try {
			...
			while (!br.ready()) {
				...
				packet[3] = xorCheck(new Message(
						MessageType.DATA,
						ByteBuffer.wrap(concatByteArrays(new byte[]{packet[0]},
								concatByteArrays(new byte[][]{new byte[]{packet[1]}, inputBytes}))));
...
>>>>>>> Stashed changes:src/Disabled/MyProtocol.java
...
>>>>>>> Stashed changes:src/Disabled/MyProtocol.java
		...
		for (int i = 0; i < bytes.length; i++) {
			System.arraycopy(bytes[i], 0, output, currentPos, bytes[i].length);
			currentPos += bytes[i].length;
		}
				...
				try {
					...
					switch (m.getType()) {
						...
						case DATA_SHORT:
							System.out.print("DATA_SHORT: ");
							printByteBuffer(m.getData(), m.getData().capacity()); //Just print the data
							System.out.println(new String(m.getData().array()).trim());
							try {
								for (int i = 0; i < m.getData().capacity(); i++) {
									System.out.print(Character.toString(m.getData().get(i)));
									/*BitSet bitset = BitSet.valueOf(m.getData());
